...
import { AbstractAuthnService } from '../abstracts/AbstractAuthnService';

@Injectable()
export class AuthnService extends AbstractAuthnService {
  constructor(
  ...
  async createChallenge(user: UserEntity) {
    try {
      const rpID = getAppConfig().AUTH.RP_ID;
  ...
  async verifyRegistration(
    ...
    try {
      const rpID = getAppConfig().AUTH.RP_ID;
      const expectedOrigin = getAppConfig().AUTH.CLIENT_URL;
  ...
  async authOptions(
    ...
    try {
      ...
      const rpID = getAppConfig().AUTH.CLIENT_URL;
